...
if (isset($_POST['send'])) {
  $name = "TriSakay Support";
  $senderEmail = htmlentities($_POST['email']);
  $subject = "Email Verification";

  // Establish a database connection and retrieve user information
  $sqlGetUserInfo = "SELECT user_id, email FROM users WHERE email = ?";
  $stmtGetUserInfo = $conn->prepare($sqlGetUserInfo);
  $stmtGetUserInfo->bind_param("s", $senderEmail);
  $stmtGetUserInfo->execute();
  $result = $stmtGetUserInfo->get_result();
  ...
  if ($result->num_rows === 1) {
    ...
    $message = '<!DOCTYPE html>
        ...
        <body>
          <h1 class="blue-text green-sakay">TriSakay</h1>
          <h2 class="title">Email Verification</h2>
          <hr>
          <h2>' . $otp . '</h2>
          <br>
          <p>This 6-digit code is to verify your email address for TriSakay account.</p>
          <p>Please copy and enter it in the <a href="http://trisakay.atwebpages.com/" target="_blank">TriSakay Website</a> to
            proceed.</p>
          <br>
          <p>Official TriSakay Representatives will never ask for this code. For your protection, please do not share this code
            with anyone.</p>
          <br>
          <p class="notice">This email contains confidential information for the sole use of the intended recipient/s. If you
            are not the intended recipient, please contact the sender, delete this email, and maintain the confidentiality of what you may have read.</p>
          <br>
          <p class="small">This is a system-generated email.<strong> Do not reply.</strong></p>
          <hr>
        </body>
        </html>';
    ...
    $mail->Password = 'grintluwuzhcpelt'; // Provide your email account password
    $mail->Port = 465;
    $mail->SMTPSecure = 'ssl';
    $mail->isHTML(true);
    ...
    $sqlUpdateStatus = "UPDATE users SET otp = ?, expiration = DATE_ADD(NOW(), INTERVAL 5 MINUTE), status = 'not used' WHERE user_id = ?";
    $stmtUpdateStatus = $conn->prepare($sqlUpdateStatus);
    $stmtUpdateStatus->bind_param("si", $hashedOtp, $user_id);

    if ($stmtUpdateStatus->execute()) {
      try {
        $mail->send();
        header("Location: ./forgot_2.php?status=email_sent"); // Redirect to forgot_2.php
...
<body >
  ...
  <div class="center">
    <h4>Enter the email address you used to create your account.</h4>
    <form  method="post">
      <div class="container d-flex justify-content-center">
        <div class="input-box">
          <div class="input-field">
            <input type="email" class="input box-shadow-primary" id="email" name="email" required autocomplete="off" />
            <label for="email"><i class="fa-solid fa-envelope fa-lg" style="color: #ffffff;"></i> Email</label>
          </div>
        </div>
      </div>
      <button type="submit" name="send" class="btn btn-default custom-btn1">
        Continue <i class="fa-solid fa-paper-plane color-white "></i>
      </button>
    </form>
  </div>
</body>
